...
###################################################
# PERMANOVA (adonis2) (*multivariate and marginal on behavioural categories included in article*)
###################################################
...
## Multivariate PERMANOVA using Bray-Curtis dissimilarity (behavioural categories) *included in article*
...
## Marginal PERMANOVA (behavioural categories) *included in article*
...
# behavioural categories *included in article*
...
#png("dirichlet_model_cat_diagnostics.png", width = 1200, height = 900, res = 150)
#plot(dirichlet_model_cat)
#dev.off()

# Residuals and fitted values for Dirichlet regression (behavioural categories)
...
plot(as.vector(fitted_vals), as.vector(resids),
     xlab = "Fitted values", ylab = "Residuals",
     main = "Residuals vs Fitted (Dirichlet Regression)")
abline(h = 0, col = "red")
dev.off()

#for (i in 1:ncol(resids)) {
#  png(sprintf("dirichlet_model_cat_residuals_qq_%s.png", colnames(resids)[i]), width = 1200, height = 900, res = 150)
#  qqnorm(resids[, i], main = paste("QQ-plot of Residuals:", colnames(resids)[i]))
...
png("dirichlet_model_cat_residuals_qq_faceted_baseR.png", width = 1800, height = 1200, res = 150)
n <- ncol(resids)
par(mfrow = c(2, ceiling(n/2))) # 2 rows, adjust columns as needed
for (i in 1:n) {
  qqnorm(resids[, i], main = paste("QQ-plot:", colnames(resids)[i]))
  qqline(resids[, i], col = "red")
}
